...
Beldex wallet backend is open sourced, free
...
to MyMonero. They include:
 ...
 - the wallets generated use 25 word mnemonics, fully compatible with official Beldex wallets
...
The Beldex Wallet consists of four components that need to be setup for it to work:
 ...
 - Beldex daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Beldex Wallet into the Beldex network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Beldex blockchain and "talks"
 to Beldex daemon.
...
Beldex Walllet is not as fast as MyMonero.
...
Below are example and basic instructions on how to setup up and run Beldex Wallet on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### Beldex download and compilation

Download and compile recent Beldex into your home folder:

```bash
# first install Beldex dependencies
sudo apt update
...
# checkout last beldex version -- v2.0.0-rc as of this edit, replace "release_tag" below with the current release tag
git checkout -b last_release release_tag

USE_SINGLE_BUILDDIR=1 make
```

#### Compilation of the Beldex Wallet (don't run it yet)

Once Beldex was downloaded and compiled, we can download Beldex Wallet and compile it.
...
git clone https://github.com/beldex-coin/beldex-web-wallet.git

cd beldex-web-wallet

mkdir build && cd build

cmake ..

# altearnatively can use cmake -DBELDEX_DIR=/path/to/beldex_folder ..
# if Beldex is not in ~/beldex
...
Create mariadb container called `beldexwalletdb` and root password of `root` (change these  how you want).

```
docker run --name beldexwalletdb -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root -d mariadb
```

Create beldexwallet database called `beldexwallet`.

```
cd beldex-web-wallet/sql
docker exec -i beldexwalletdb mysql -uroot -proot < db.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the beldexwallet database is through the
[PhpMyAdmin in docker](https://hub.docker.com/r/phpmyadmin/phpmyadmin/). Using docker,
this can be done:

```
docker run --name myadmin -d --link beldexwalletdb:db -p 8080:80 phpmyadmin/phpmyadmin
```

where `beldexwalletdb` is the name of docker container with mariadb, set in previous step.
...
```
docker run --name html -p 80:80 -v /home/username/beldexwallet/html:/usr/share/nginx/html:ro -d nginx
```

where `beldexhtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/username/beldex-web-wallet/html` is the location on your host computer where the
...
Before running `beldex-web-wallet`:
...
```bash
./beldex-web-wallet -s -c /path/to/config.json
```


## Beldex Locker JSON REST API

Example JSON REST requests and their responses of [Beldex Wallet](https://github.com/beldex-coin/beldex-web-wallet) are provided below. The long term goal
...
the Beldex Locker API does not fully conform to that.

#### get_version

Get version of the Beldex Wallet, its API and Beldex.
...
```json
{
  ...
  "beldex_version_full": "0.2.0-master"
}
```
...
Login an existing or new user into Beldex Wallet

```bash
curl  -w "\n" -X POST http://127.0.0.1:1984/login -d '{"address": "82VTvE8bC9APsWFn3mQzgW8Xfcy2SP2CRUArD6ZtthNaWDuuvyhtBcZ8WDuYMRt1HhcnNQvpXVUavEiZ9waTbyBhP6RM8TV", "view_key": "041a241325326f9d86519b714a9b7f78b29111551757eeb6334d39c21f8b7400"}'
```
...
```bash
curl  -w "\n" -X POST http://127.0.0.1:1984/get_address_txs -d '{"address": "82VTvE8bC9APsWFn3mQzgW8Xfcy2SP2CRUArD6ZtthNaWDuuvyhtBcZ8WDuYMRt1HhcnNQvpXVUavEiZ9waTbyBhP6RM8TV", "view_key": "041a241325326f9d86519b714a9b7f78b29111551757eeb6334d39c21f8b7400"}'
```
...
```bash
curl  -w "\n" -X POST http://127.0.0.1:1984/get_address_info -d '{"address": "82VTvE8bC9APsWFn3mQzgW8Xfcy2SP2CRUArD6ZtthNaWDuuvyhtBcZ8WDuYMRt1HhcnNQvpXVUavEiZ9waTbyBhP6RM8TV", "view_key": "041a241325326f9d86519b714a9b7f78b29111551757eeb6334d39c21f8b7400"}'
```
...
Get random outputs from the Beldex daemon to be used as ring members
...
Submit raw tx in hex generated by the frontend to be relayed to the Beldex network.
