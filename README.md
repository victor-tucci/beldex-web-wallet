# Beldex Web Wallet - a lightweight web wallet for Beldex based on OpenMonero 
...
demonstrate how to provide Beldex related JSON REST service. For this purpose,
 a service called Beldex Web Wallet was developed.


Beldex Web Wallet is an open source implementation of backend of
https://mymonero.com/ and https://loki.networks adapted for Beldex. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://mymonero.com/.

Beldex Web Wallet backend is open sourced, free
...
[branch](https://github.com/Beldex-Coin/beldex-web-wallet/tree/dev).


## Screenshot

![Beldex Web Wallet](screenshot/screen1.png)


## Host it yourself

The Beldex Web Wallet consists of four components that need to be setup for it to work:
 ...
 from the Beldex Web Wallet into the Beldex network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Beldex blockchain and "talks" to Beldex daemon.
...
Beldex Web wallet is not as fast as MyMonero.
...
Below are example and basic instructions on how to setup up and run Beldex Web  Wallet on Ubuntu 16.04.
...
# checkout last Beldex version -- v3.1.4-rc as of this edit, replace "release_tag" below with the current release tag
git checkout -b last_release release_tag

USE_SINGLE_BUILDDIR=1 make
```

#### Compilation of the Beldex Web Wallet (don't run it yet)

Once Beldex was downloaded and compiled, we can download Beldex Web wallet and compile it.
...
Create mariadb container called `beldexlockerdb` and root password of `root` (change these  how you want).

```
docker run --name beldexlockerdb -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root -d mariadb
```

Create beldexlocker database called `beldexlocker`.

```
cd beldexlocker/sql
docker exec -i beldexlockerdb mysql -uroot -proot < db.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the beldexlocker database is through the
[PhpMyAdmin in docker](https://hub.docker.com/r/phpmyadmin/phpmyadmin/). Using docker,
this can be done:

```
docker run --name myadmin -d --link beldexlockerdb:db -p 8080:80 phpmyadmin/phpmyadmin
```

where `beldexlockerdb` is the name of docker container with mariadb, set in previous step.
...
```
docker run --name beldexhtml -p 80:80 -v /home/username/beldexlocker/html:/usr/share/nginx/html:ro -d nginx
```

where `beldexhtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/username/beldexlocker/html` is the location on your host computer where the
...
## Beldex Web Wallet JSON REST API

Example JSON REST requests and their responses of [Beldex Web Wallet](https://github.com/Beldex-Coin/beldex-web-wallet) are provided below. The long term goal
...
the Beldex Web Wallet API does not fully conform to that.

#### get_version

Get version of the Beldex Web Wallet, its API and Beldex.
