...
demonstrate how to provide Loki related JSON REST service. For this purpose,
 a service called Loki Locker was developed.


Loki Locker is an open source implementation of backend of
https://mymonero.com/ adapted for Loki. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://mymonero.com/.

Loki Locker's backend is open sourced, free
...
to MyMonero. They include:
 ...
 - the wallets generated use 25 word mnemonics, fully compatible with official Loki wallets
...
The Loki Locker consists of four components that need to be setup for it to work:
 ...
 - Loki daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Loki Locker into the Loki network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Loki blockchain and "talks"
 to Loki daemon.
...
Loki Locker is not as fast as MyMonero.
...
Below are example and basic instructions on how to setup up and run Loki Locker on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### Loki download and compilation

Download and compile recent Loki into your home folder:

```bash
# first install Loki dependencies
sudo apt update
...
# checkout last Loki version -- v2.0.0-rc as of this edit, replace "release_tag" below with the current release tag
git checkout -b last_release release_tag

USE_SINGLE_BUILDDIR=1 make
```

#### Compilation of the Loki Locker (don't run it yet)

Once Loki was downloaded and compiled, we can download Loki Locker and compile it.
...
work without database, setup frontend, and synced and running Loki blockchain.
...
git clone https://github.com/loki-project/loki-locker.git
...
# altearnatively can use cmake -DLOKI_DIR=/path/to/loki_folder ..
...
Create mariadb container called `lokilockerdb` and root password of `root` (change these  how you want).

```
docker run --name lokilockerdb -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root -d mariadb
```

Create lokilocker database called `lokilocker`.

```
cd lokilocker/sql
docker exec -i lokilockerdb mysql -uroot -proot < db.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the lokilocker database is through the
[PhpMyAdmin in docker](https://hub.docker.com/r/phpmyadmin/phpmyadmin/). Using docker,
this can be done:

```
docker run --name myadmin -d --link lokilockerdb:db -p 8080:80 phpmyadmin/phpmyadmin
```

where `lokilockerdb` is the name of docker container with mariadb, set in previous step.
...
```
docker run --name lokihtml -p 80:80 -v /home/username/lokilocker/html:/usr/share/nginx/html:ro -d nginx
```

where `lokihtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/username/lokilocker/html` is the location on your host computer where the
...
Before running `loki-locker`:
 ...
 - make sure Loki daemon is running and fully sync. If using testnet or stagenet networks, use Loki daemon
...
```bash
./loki-locker -s -c /path/to/config.json
```


## Loki Locker JSON REST API

Example JSON REST requests and their responses of [Loki Locker](https://github.com/loki-project/loki-locker) are provided below. The long term goal
...
the Loki Locker API does not fully conform to that.

#### get_version

Get version of the Loki Locker, its API and Loki.
...
```json
{
  ...
  "loki_version_full": "0.2.0-master"
}
```
...
Login an existing or new user into Loki Locker
...
Get random outputs from the Loki daemon to be used as ring members
...
Submit raw tx in hex generated by the frontend to be relayed to the Loki network.
